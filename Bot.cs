...
namespace SteamBot
{
    public class Bot
    {
        ...
        // ================================
        // Start with the autocrafting crap
        // ================================
        public bool AutoCraftAllWeps()
        {
            // Put me in Bot.cs!
            GetInventory();
            ...
            while (canCraft)
            {
                if (iterate > 8)
                {
                    if (numCrafted == 0) break;
                    numCrafted = 0;
                    iterate = 0;
                    GetInventory();
                }
                string currentClass = TF2Class[iterate];
                log.Info("Looking for " + currentClass + " weapons to craft...");
                foreach (var item in MyInventory.Items)
                {
                    var schemaItem = Trade.CurrentSchema.GetItem(item.Defindex);
                    if (schemaItem.CraftClass == "weapon" || schemaItem.CraftMaterialType == "weapon")
                    {
                        ...
                        {
                            log.Info("Adding " + schemaItem.ItemName + " to craft list.");
                            toCraft.Add(item.Id);
                        }
                        if (toCraft.Count == 2)
                            break;
                ...
                if (toCraft.Count == 2)
                {
                    ...
                    TF2GC.Crafting.CraftItems(this, (short)TF2GC.ECraftingRecipe.SmeltClassWeapons, toCraft.ToArray());
                    //TF2GC.Crafting.CraftItems(this, (short)TF2GC.ECraftingRecipe.SmeltClassWeapons, toCraft.ToArray());
                    numCrafted++;
                    Thread.Sleep(200);
                }
                else if(toCraft.Count !=2 || toCraft.Count !=1)
                {
                    log.Info("Not enough " + currentClass + " weapons to craft; clearing list.");
                    log.Info("BlockCat is the best.");
                }
                toCraft.Clear();
                iterate++;
            }
            SetGamePlaying(0);
            return numCrafted > 0 ? true : false;
        }
        public bool AutoCraftAllScrap()
        {
            // Put me in Bot.cs!
            GetInventory();
            List<ulong> toCraft = new List<ulong>();
            ...
            while (canCraft)
            {
                ...
                {
                    var schemaItem = Trade.CurrentSchema.GetItem(item.Defindex);
                    if (schemaItem.Defindex == 5000)
                    {
                        if (!item.IsNotTradeable && !item.IsNotCraftable && !toCraft.Contains(item.Id))
                        {
                            log.Info("Adding " + schemaItem.ItemName + " to craft list.");
                            toCraft.Add(item.Id);
                        }
                ...
                if (toCraft.Count == 3)
                {
                    ...
                    TF2GC.Crafting.CraftItems(this, (short)TF2GC.ECraftingRecipe.CombineScrap, toCraft.ToArray());
                    numCrafted++;
                    Thread.Sleep(200);
                }
                else
        ...
        public bool AutoCraftAllRec()
        {
            ...
            while (canCraft)
            {
                ...
                if (toCraft.Count == 3)
                {
                    ...
                    TF2GC.Crafting.CraftItems(this, (short)TF2GC.ECraftingRecipe.CombineReclaimed, toCraft.ToArray());
                    numCrafted++;
                    Thread.Sleep(200);
                }
                else
